 ...
 *    actually exists on disk — the automated check for "no fake complete status."
...
// This is the automated enforcement of "no fake complete status": a task marked
// [x] that names a file which doesn't exist means either the doc is lying or the
// file was deleted/renamed after the fact without updating the claim.
console.log(
	...
	if (!fs.existsSync(testDirPath)) {
		...
		for (const tf of testFiles) {
			...
			const inBuildConfig = buildConfigContent
				...
				: true; // no build config configured — skip this half of the check
			const testScript =
